...
function GitHubStarButton({ owner, repo }) {
  ...
  const handleStarClick = async () => {
    try {
      if (isStarred) {
        ...
        await axios.put(`https://api.github.com/user/starred/${owner}/${repo}`, null, {
          headers: {
            Authorization: `Bearer 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`,
          },
        });
  ...
  return (
    <div className='text-center justify-center items-center flex flex-col'>
      <button onClick={handleStarClick} className='github-star-button flex justify-center items-center text-center p-1 h-6 m-0'>
        {isStarred ? '⭐' : '⭐'}
      </button>
      <p className='text-xs font-bold drop-shadow-sm'>{starCount} stars</p>
    </div>
  );
}
